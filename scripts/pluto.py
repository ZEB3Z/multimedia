"""Pluto — Keplerian ellipse (e ≈ 0.249); safe to re-run."""
...
def main():
    ...
    d = sc.PLANETS[BODY]
    a_bu = sc.orbit_bu(d["semi_major_axis_au"])
    e = float(d["eccentricity"])
    phi = math.radians(float(d["start_angle_deg"]))
    sc.create_pluto_elliptical_orbit_ring(
        a_bu,
        e,
        f"OrbitRing_{BODY}",
        d["ring_color"],
    ...
    sc.build_pluto_elliptical(d)
    bpy.context.scene.frame_set(1)
    print(f"✅ {BODY} added — elliptical orbit, texture: {sc.texture_path_for_key(d['key'])}.")
